...
export const globalInit = async () => {
    ...
    const componentsCoreRemote = {
        ...registry, remote: "components-core", defaultVersion: '0.2.9',
    };
    remoteLoaderPreset({
        remotes: {
    ...
    const ajaxPostForm = async (url, data, options) => {
        const {default: getToken} = await loadModule('components-account:Account@getToken');
        const token = getToken();
    ...
    return {
        ajax, ajaxPostForm, staticUrl: baseApiUrl, apis: Object.assign(
            {},
            {
                account: getApis({prefix: '/api/v1/account'}),
                oss: {
                ...
                ossUpload: async ({file}) => {
                    return await ajaxPostForm('/api/v1/static/upload', {file});
                }
            },
            {project: apis}
        ),
        permissionsPath: 'userInfo.tenantUser.permissions', themeToken: {
